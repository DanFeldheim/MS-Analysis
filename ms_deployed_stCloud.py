...
class Load_Data():
    ...
    def import_to_df(self, uploaded_files, select_pfas):
        """
        Lazily reads uploaded .mzML files and filters spectra on-the-fly
        according to PFAS retention time and m/z ranges.
        Returns a dictionary of DataFrames keyed by filename.
        """
    
        if not uploaded_files or not select_pfas:
            return None
        ...
        for file in uploaded_files:
            ...
            # Use a generator to stream filtered spectra
            def filtered_spectra():
                with mzml.MzML(file_path) as reader:
                    for spectrum in reader:
                        ...
                        # Yield rows one at a time instead of building a large list
                        for m, intensity in zip(mz_array, intensity_array):
                            if mz_min <= m <= mz_max:
                                yield {
                                    'retention_time': rt,
            ...
            # Convert generator to DataFrame in chunks
            chunk_size = 5000  # number of rows per chunk
            chunks = []
            temp_rows = []
            for i, row in enumerate(filtered_spectra(), 1):
                temp_rows.append(row)
                if i % chunk_size == 0:
                    chunks.append(pd.DataFrame(temp_rows))
                    temp_rows = []
            if temp_rows:
                chunks.append(pd.DataFrame(temp_rows))
            ...
            if chunks:
                ...
                df = pd.DataFrame(columns=['retention_time', 'mz', 'intensity'])
